...
//Using the split() string method to create an array of ordered chars, including spaces
    ...
    if (str1.length == str2.length){ //if the lengths of the strings are equal
        for (let i = 0; i < strArray1.length; i++){ //iterate through both arrays based off the length of strArray1 (amount of chars in str1)
            strArray1[i] !== strArray2[i] ? result += 1 : result; //Is the value at strArray1 at index i not the same as strArray2 at index i? If so, result is equal to itself plus 1, otherwise result remains the same.
        }
    ...
    return `There are ${result} differences!`; //return a string that displays the value in 'result' with context; 'There are this amount of differences'
}
